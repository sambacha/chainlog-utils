 ...
 */
const main = async () => {
  const contractArtifactsFolder = path.resolve(__dirname, `../artifacts/contracts`);

  const artifactPaths = glob.sync(`${contractArtifactsFolder}/**/*.json`).filter((match) => {
  ...
  fs.writeFileSync(path.resolve(__dirname, `../src/contract-artifacts.ts`), content);
};
