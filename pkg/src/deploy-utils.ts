...
export const getAdvancedContract = (opts: { hre: any; contract: Contract }): Contract => {
...
export const fundAccount = async (hre: any, address: string, amount: ethers.BigNumber) => {
  const deployConfig = getDeployConfig(hre.network.name);
  ...
  await hre.ethers.provider.send('hardhat_setBalance', [address, amount.toHexString()]);
...
export const sendImpersonatedTx = async (opts: {
  ...
  await opts.hre.ethers.provider.send('hardhat_stopImpersonatingAccount', [opts.from]);
};
