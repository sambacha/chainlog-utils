...
export const color = Object.fromEntries(
  Object.entries(codes).map(([k]) => [k, (msg: string) => `${codes[k]}${msg}${codes.reset}`]),
);
...
export const printSectionHead = (msg: string) => {
  ...
  console.log(color.cyan('='.repeat(Math.max(...msg.split('\n').map((s) => s.length)))));
};
...
export const printComparison = (
  ...
  if (hexStringEquals(expected.value, deployed.value)) {
    console.log(color.green(`${expected.name}: ${truncateLongString(expected.value)}`));
    console.log('matches');
    console.log(color.green(`${deployed.name}: ${truncateLongString(deployed.value)}`));
